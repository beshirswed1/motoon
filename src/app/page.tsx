...
import {
  ...
  Languages, Landmark, Megaphone, HelpCircle
} from 'lucide-react';

export async function generateMetadata(): Promise<Metadata> {
  return createPageMetadata({
    title: '🟢 متون | الموسوعة التعليمية لحفظ المتون الشرعية',
    description: '📖 احفظ المتون الشرعية وتتبع تقدمك في الحفظ بنظام التكرار المتباعد الذكي والتسميع الصوتي التفاعلي بالذكاء الاصطناعي. تصفح متون الفقه، العقيدة، الحديث، النحو والقرآن.',
    keywords: [
      'متون',
      'حفظ المتون الشرعية',
      'منصة تعليم إسلامي',
...
const homepageFaqs = [
  {
    question: 'ما هي منصة متون؟',
    answer: 'متون هي منصة وتطبيق تعليمي إسلامي لمساعدة طلاب العلم في حفظ وتسميع ومراجعة المتون الشرعية باستخدام تقنيات الذكاء الاصطناعي وخوارزميات التكرار المتباعد.',
  },
  {
    question: 'هل منصة متون مجانية؟',
    answer: 'نعم، منصة متون مجانية تماماً وبدون إعلانات مزعجة لخدمة كافة طلاب العلم الشرعي في العالم الإسلامي.',
  },
  {
    question: 'هل يمكن استخدام تطبيق متون بدون إنترنت؟',
    answer: 'نعم، المنصة مبنية كتطبيق ويب تقدمي (PWA) يعمل بكفاءة كاملة بدون اتصال بالإنترنت مع مزامنة التقدم فور عودة الاتصال.',
  },
  {
    question: 'كيف أتحصل على شهادة إتمام المتن؟',
    answer: 'عند إكمالك للحفظ والتسميع الصوتي لأي متن بدرجة إتقان 95% أو أعلى تترشح تلقائياً لاستخراج شهادة إتمام برمز تحقق فريد.',
  },
];
...
export default async function HomePage() {
  ...
  const howToSchema = createHowToSchema({
    name: 'كيفية حفظ المتون الشرعية عبر منصة متون',
    description: 'خطوات أربع مرسمة لحفظ المتون العلمية واستحقاق شهادة الإتمام.',
    steps: howToSteps.map((s) => ({ name: s.title, text: s.desc })),
  });
  ...
  return (
    <div className="flex flex-col w-full">
      {/* Dynamic Homepage Structured Data */}
      <JsonLd data={pageSchemas} />

      {/* ─── Hero Section ─────────────────────────── */}
      <section
        aria-label="الرئيسية والترحيب"
        className="relative overflow-hidden py-20 md:py-28 section-padding"
      >
        ...
        <div className="container-motoon relative z-10 flex flex-col items-center text-center gap-8">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 border border-primary/20 text-primary text-sm font-bold">
            ...
            منصة حفظ المتون العلمية الأولـى
          </div>
          ...
          <h1 className="max-w-4xl text-4xl md:text-6xl font-black tracking-tight text-foreground leading-tight">
            ...
            <span className="relative inline-block">
              ...
              >
                <path
                  d="M2 8C50 2 100 10 150 6S250 2 298 8"
                  stroke="currentColor"
                  strokeWidth="3"
                  strokeLinecap="round"
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <Button size="lg" variant="outline" asChild className="text-base font-bold rounded-xl px-8 h-12 border-border/60">
              <Link href="/sciences">
                <Languages className="h-4 w-4 ml-2 text-primary" />
          ...
          <div className="flex flex-wrap items-center justify-center gap-6 text-sm text-muted-foreground mt-2">
            {[
              ...
              { icon: Zap, label: 'بدون إنترنت (PWA)' },
            ].map(({ icon: Icon, label }) => (
      ...
      {carouselBooks.length > 0 && (
        <section aria-label="أبرز المتون" className="py-16 section-padding">
          <div className="container-motoon">
            <div className="mb-8 flex items-end justify-between">
              <div>
                <p className="text-sm font-bold text-primary uppercase tracking-widest mb-1">مميز</p>
                <h2 className="text-2xl md:text-3xl font-black">أبرز المتون العلمية</h2>
      ...
      {/* ─── Categories Section ───── */}
      <section aria-label="أقسام العلوم الشرعية" className="py-16 section-padding bg-muted/30">
        <div className="container-motoon">
                ...
                >
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-[10px] md:text-xs text-muted-foreground">
                      {cat.subcategories.length} تخصص علمي
                    </p>
                  </div>
                </Link>
              );
      ...
      {/* ─── How It Works ──────────────────────────── */}
      <section aria-label="طريقة الحفظ" className="py-20 section-padding">
        <div className="container-motoon">
      ...
      {/* ─── Platform Features ─────────────────────── */}
      <section aria-label="مميزات المنصة" className="py-20 section-padding bg-muted/30">
        <div className="container-motoon">
          <div className="text-center mb-14">
            <p className="text-sm font-bold text-primary uppercase tracking-widest mb-2">المميزات</p>
            <h2 className="text-2xl md:text-3xl font-black mb-3">لماذا منصة متون؟</h2>
            <p className="text-muted-foreground max-w-2xl mx-auto">
              منصة شاملة تجمع أحدث أساليب التعليم الذكي مع الأصالة الإسلامية
            </p>
          </div>
      ...
      {/* ─── Statistics Section ────────────────────── */}
      <section aria-label="إحصائيات المنصة" className="py-20 section-padding bg-primary text-primary-foreground relative overflow-hidden">
        ...
        <div className="container-motoon relative z-10">
          <div className="text-center mb-12">
            ...
            <p className="opacity-80">أرقام حقيقية تعكس ثقة طلاب العلم الشرعي</p>
          </div>
      ...
      {/* ─── FAQ Section ──────────────────────────── */}
      <section aria-label="الأسئلة الشائعة" className="py-20 section-padding">
        <div className="container-motoon max-w-4xl">
          <div className="text-center mb-12">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 text-primary text-xs font-bold mb-3">
              <HelpCircle className="h-3.5 w-3.5" /> الأسئلة الشائعة
            </div>
            <h2 className="text-2xl md:text-3xl font-black mb-2">أسئلة يتكرر طرحها</h2>
            <p className="text-muted-foreground text-sm">كل ما تود معرفته عن منصة متون وتطبيقاتها</p>
          </div>

          <div className="grid gap-4">
            {homepageFaqs.map((faq, idx) => (
              <div key={idx} className="p-6 rounded-2xl border border-border/60 bg-card shadow-sm">
                <h3 className="font-bold text-lg text-foreground mb-2 flex items-center gap-2">
                  <span className="text-primary font-mono text-sm">Q:</span>
                  {faq.question}
                </h3>
                <p className="text-muted-foreground text-sm leading-relaxed pr-6">{faq.answer}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ─── Certificate Section ───────────────────── */}
      <section aria-label="شهادات المنصة" className="py-16 section-padding bg-muted/30">
        <div className="container-motoon">
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-amber-50 to-amber-100/50 dark:from-amber-950/20 dark:to-amber-900/10 border border-amber-200/50 dark:border-amber-800/30 p-8 md:p-14 text-center">
            <div className="relative z-10 flex flex-col items-center gap-5 max-w-xl mx-auto">
              ...
              <p className="text-muted-foreground leading-relaxed">
                ...
                تحصل على شهادة إتمام احترافية بالأسلوب الإسلامي تحمل اسمك ودرجتك برمز تحقق فريد.
              </p>
